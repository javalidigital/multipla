<?php
class kraftive_latest_tweets extends WP_Widget
{
    /**
    ...
    {
        parent::__construct(
            'kraft_twitter', // Base ID
            'Kraftive: Latest Tweets', // Name
            array('description' => __('Displays your latest Tweets', 'kraftives'),) // Args
        );
    }
    ...
    public function widget($args, $instance) {
        ...
        if(empty($instance['consumerkey']) || empty($instance['consumersecret']) || empty($instance['accesstoken']) || empty($instance['accesstokensecret']) || empty($instance['cachetime']) || empty($instance['username'])){
            _e('<div class="folio-alert yellow">Please fill all widget settings!</div>' ,'kraftives');
			echo $after_widget;
            return;
        }
        ...
        if($diff >= $crt || empty($tp_twitter_plugin_last_cache_time)){
            ...
            if(!empty($tweets->errors)){
                if($tweets->errors[0]->message == 'Invalid or expired token'){
					$twitter_url = 'https://dev.twitter.com/apps';
                    echo '<strong>'.$tweets->errors[0]->message.'!</strong><br />You\'ll need to regenerate it <a href="'.$twitter_url.'" target="_blank">here</a>!' . $after_widget;
                }else{
        ...
        if(!empty($tp_twitter_plugin_tweets)){
            ...
            foreach($tp_twitter_plugin_tweets as $tweet){
                ...
                print '<div class="tweet"><span class="icon"><i class="fa fa-twitter"></i></span>'.convert_links($tweet['text']).'<br/><div class="time">'.relative_time($tweet['created_at']).'</div></div>';
    ...
    public function update($new_instance, $old_instance) {
        ...
        $instance['consumerkey'] = wp_filter_nohtml_kses(strip_tags( $new_instance['consumerkey'] ));
        $instance['consumersecret'] = wp_filter_nohtml_kses(strip_tags( $new_instance['consumersecret'] ));
        $instance['accesstoken'] = wp_filter_nohtml_kses(strip_tags( $new_instance['accesstoken'] ));
        $instance['accesstokensecret'] = wp_filter_nohtml_kses(strip_tags( $new_instance['accesstokensecret'] ));
        $instance['cachetime'] = wp_filter_nohtml_kses(strip_tags( $new_instance['cachetime'] ));
        $instance['username'] = wp_filter_nohtml_kses(strip_tags( $new_instance['username'] ));
        $instance['tweetstoshow'] = wp_filter_nohtml_kses(strip_tags( $new_instance['tweetstoshow'] ));
