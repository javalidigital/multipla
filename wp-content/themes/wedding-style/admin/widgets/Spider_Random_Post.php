<?php
/// random post widget
$wedding_style_spider_random_post_id=0;
if(!class_exists('wedding_style_spider_random_post')){
	class wedding_style_spider_random_post extends WP_Widget {
	// Constructor //
		function wedding_style_spider_random_post() {
			$widget_ops = array( 'classname' => 'wedding_style_spider_random_post', 'description' => 'Spider Random Post allows you to show posts in a random order in a sidebar.' ); // Widget Settings
			$control_ops = array( 'id_base' => 'wedding_style_spider_random_post' ); // Widget Control Settings
			$this->WP_Widget( 'wedding_style_spider_random_post', 'Spider Random Post', $widget_ops, $control_ops ); // Create the widget
		}
	// Extract Args //
		function widget($args, $instance) {
			...
			$url = plugins_url();
			global $wedding_style_spider_random_post_id;
	// Before widget //
	...
	// Widget output //
if($wedding_style_spider_random_post_id==0){
...
<?php 
  ...
  ?>
<div  id="randarticle_<?php echo $wedding_style_spider_random_post_id ?>" >
<?php
global $wedding_style_spider_random_post_id;
echo "<script type='text/javascript'> Update(".$wedding_style_spider_random_post_id.",".$instance['Updating_Time'].",".$instance['Category'].",".$instance['quantity_of_posts'].",".$instance['Style_sra'].");  </script>";
	
$wedding_style_spider_random_post_id++;   
?>
</div>
<?php
	// After widget //
...
{
?>
<option value="<?php if(isset($categories[$i])) echo $categories[$i]->term_id?>" <?php if(isset($categories[$i]) && $instance['Category']==$categories[$i]->term_id) echo  'selected="selected"'; ?>><?php if(isset($categories[$i])) echo $categories[$i]->name ?></option>
<?php
}
 ?>
</select>
...
// End class wedding_style_spider_random_post
add_action('widgets_init', create_function('', 'return register_widget("wedding_style_spider_random_post");'));
add_action('wp_ajax_sprandom_post', 'wedding_style_spider_random_post_fornt_end');
add_action('wp_ajax_nopriv_sprandom_post', 'wedding_style_spider_random_post_fornt_end');
function wedding_style_spider_random_post_fornt_end(){
